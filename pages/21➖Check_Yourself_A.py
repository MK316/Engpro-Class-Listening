...
# --- 1. Timezone & Page Configuration ---
KST = pytz.timezone('Asia/Seoul')
st.set_page_config(page_title="Listening Exercise A", page_icon="🎱", layout="centered")

# --- 2. PDF Generation Logic ---
class PDF(FPDF):
    def header(self):
        # Decorative Navy Header
...
def create_pdf(name, score, total, results, start_t, end_t):
    ...
    pdf.cell(0, 7, f"Session Started: {start_t} (KST)", 0, 1)
    pdf.cell(0, 7, f"Session Ended: {end_t} (KST)", 0, 1)
...
# --- 3. Session State for Start Time ---
if 'start_time' not in st.session_state:
    st.session_state.start_time = datetime.now(KST).strftime("%Y-%m-%d %H:%M:%S")

# --- 4. Sidebar & Identity ---
st.sidebar.header("📋 Student Identification")
user_name = st.sidebar.text_input("Enter your full name:", placeholder="e.g., Hong Gil-dong")

# --- 5. Main UI & Instructions ---
...
st.info("""
**[Practice Example]**
- Audio: *1. mitt, 2. meat, 3. meat*
- Selection: Choose **1** (mitt)
""")
...
# --- 7. Submission & PDF Export ---
if st.button('Finish Exercise & Generate PDF Report'):
    if not user_name.strip():
        st.warning("⚠️ Please enter your name in the sidebar before finishing.")
    else:
        # Capture End Time
        ...
        st.success(f"🎉 Well done, {user_name}! Your score is {score}/10.")
