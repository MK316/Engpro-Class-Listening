...
st.set_page_config(page_title="Check Yourself C", page_icon="🎱", layout="centered")

# --- 2. PDF 생성 로직 (Vertical Header Fix 적용) ---
class PDF(FPDF):
    def header(self):
        ...
        self.set_y(0)
        self.cell(0, 20, 'Lesson 2 Check Yourself C: Sentence Completion Report', 0, 1, 'C')
        self.ln(5)
...
def create_pdf(name, score, total, results, start_t, end_t):
    ...
    for q_num, user_ans, correct_ans, is_correct in results:
        pdf.cell(20, 10, f"Q{q_num}", 1, 0, 'C')
...
def display_instructions():
    ...
    st.write("Listen to the sentences. Choose the word that completes each sentence correctly.")
    st.info("""
    **[Practice Example]**
    - You hear: *'You need a new wheel.'*
    - Correct Choice: **wheel**
    """)
...
st.markdown("### 🎱 Check Yourself C: Sentence Completion")
...
if not st.session_state.exercise_started:
    ...
    audio_url = 'https://raw.githubusercontent.com/MK316/Engpro-Class-Listening/main/audio/L01C.wav'
    ...
    if st.button('Finish & Generate PDF Report'):
        ...
        st.success(f"Exercise Submitted! Your Score: {score}/10")
        ...
        st.download_button(
            ...
            data=pdf_bytes,
            file_name=f"ExerciseC_{st.session_state.user_name}.pdf",
